...
import fr.legrain.gestCom.Module_Document.SWTEtat;
...
public class SWTPaEtatController extends JPABaseControllerSWTStandard
implements RetourEcranListener {

	static Logger logger = Logger.getLogger(SWTPaEtatController.class.getName());
	private PaEtatSWT vue = null;
	private TaEtatDAO dao = null;//new TaEtatDAO(getEm());

	private Object ecranAppelant = null;
	private SWTEtat swtEtat;
	private SWTEtat swtOldEtat;
	private Realm realm;
	private DataBindingContext dbc;

	private Class classModel = SWTEtat.class;
	private ModelGeneralObjet<SWTEtat,TaEtatDAO,TaEtat> modelEtat = null;//new ModelGeneralObjet<SWTEtat,TaEtatDAO,TaEtat>(dao,classModel);
	...
	private LgrDozerMapper<SWTEtat,TaEtat> mapperUIToModel = new LgrDozerMapper<SWTEtat,TaEtat>();
	...
	public SWTPaEtatController(PaEtatSWT vue,EntityManager em) {
		...
		dao = new TaEtatDAO(getEm());
		modelEtat = new ModelGeneralObjet<SWTEtat,TaEtatDAO,TaEtat>(dao,classModel);
		setVue(vue);
...
//			tableViewer.setInput(writableList);
	...
	private void changementDeSelection() {
		if(selectedEtat!=null && selectedEtat.getValue()!=null) {
			if(((SWTEtat) selectedEtat.getValue()).getIdEtat()!=null) {
				taEtat = dao.findById(((SWTEtat) selectedEtat.getValue()).getIdEtat());
			}
		...
		try {	
			if (selectedEtat.getValue()!=null){
				...
				if(!containsEntity(taEtat)) 
					modelEtat.getListeEntity().add(taEtat);
				changementDeSelection();
				this.swtOldEtat=SWTEtat.copy((SWTEtat)selectedEtat.getValue());
			}
			return true;
		} catch (Exception e) {
			return false;
...
//			if(LgrMess.isDOSSIER_EN_RESEAU())dao.refresh(dao.findById(((SWTEtat) selectedUnite.getValue()).getIdUnite()));
//			taUnite=dao.findById(((SWTEtat) selectedUnite.getValue()).getIdUnite());
//			try {
//				if(LgrMess.isDOSSIER_EN_RESEAU())actRefresh();
//			} catch (Exception e) {
//				logger.error("",e);
//			}			
//			this.swtOldUnite=SWTEtat.copy((SWTEtat)selectedUnite.getValue());
//		}
//	}
		...
		try {
			...
			if(isUtilise())MessageDialog.openInformation(vue.getShell(), MessagesEcran
				...
				if (MessageDialog.openConfirm(vue.getShell(), MessagesEcran
					...
					TaEtat u = dao.findById(((SWTEtat) selectedEtat.getValue()).getIdEtat());
					dao.supprimer(u);
...
//
//		fonctionGetInfosXmlAndProperties.findInfosFileXml(TaEtat.class.getName(),SWTEtat.class.getName(),
//				listeChamp,"mapping");
	...
	protected void actAide(String message) throws Exception {
		if(aideDisponible()){
			try {
				...
				switch (SWTPaEtatController.this.dao.getModeObjet().getMode()) {
				case C_MO_CONSULTATION:
					if(getFocusCourantSWT().equals(vue.getGrille())){
...
//			} else {
				...
				if(((SWTEtat) selectedEtat.getValue()).getIdEtat()!=null) {
					u.setIdEtat(((SWTEtat) selectedEtat.getValue()).getIdEtat());
				}
...
//			}
	...
	protected void actEnregistrer() throws Exception {
			...
			if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
				LgrDozerMapper<SWTEtat,TaEtat> mapper = new LgrDozerMapper<SWTEtat,TaEtat>();
				mapper.map((SWTEtat) selectedEtat.getValue(),taEtat);
...
//				modelUnite.getListeEntity().add(taUnite);
	...
	public boolean isUtilise(){
		return (((SWTEtat)selectedEtat.getValue()).getIdEtat()!=null &&
		!dao.recordModifiable(dao.getNomTable(),
				((SWTEtat)selectedEtat.getValue()).getIdEtat()))||
				!dao.autoriseModification(taEtat);		
	}
	public SWTEtat getSwtOldEtat() {
		return swtOldEtat;
	}

	public void setSwtOldEtat(SWTEtat swtOldUnite) {
		...
		if (selectedEtat.getValue() != null)
			...
			if(tableViewer.selectionGrille(0)){
				...
				tableViewer.setSelection(new StructuredSelection(
						(SWTEtat) selectedEtat.getValue()), true);
			}
		}
	}
	...
	protected void actRefresh() throws Exception {
		...
		} else if(selectedEtat!=null && (SWTEtat) selectedEtat.getValue()!=null) {
			idActuel = ((SWTEtat) selectedEtat.getValue()).getIdEtat();
		}
		...
		if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION) == 0) {
			...
			if (taEtat!=null && selectedEtat!=null && (SWTEtat) selectedEtat.getValue()!=null) {
				new LgrDozerMapper<TaEtat, SWTEtat>().
					map(taEtat, (SWTEtat) selectedEtat.getValue());
			}
		}
	...
	public ModelGeneralObjet<SWTEtat,TaEtatDAO,TaEtat> getModelEtat() {
		return modelEtat;
	}

	public TaEtatDAO getDao() {
		return dao;
	}

...
public class SWTPaFournisseurController extends JPABaseControllerSWTStandard
	...
	private SWTTiers swtFournisseurArticle;
	private SWTTiers swtOldFournisseurArticle;
	private Realm realm;
	private DataBindingContext dbc;

	private Class classModel = SWTTiers.class;
	private ModelGeneralObjet<SWTTiers,TaTiersDAO,TaTiers> modelFournisseurArticle = null;
	...
	public SWTPaFournisseurController(PaFournisseurSWT vue,EntityManager em) {
		...
		modelFournisseurArticle = new ModelGeneralObjet<SWTTiers,TaTiersDAO,TaTiers>(dao,classModel);
		setVue(vue);
	...
	public List<SWTTiers> IHMmodel() {
		...
		if(masterEntity!=null && !masterEntity.getTaFournisseurs().isEmpty()) {
			...
			LgrDozerMapper<TaTiers,SWTTiers> mapper = new LgrDozerMapper<TaTiers,SWTTiers>();
			for (TaTiers o : ldao) {
				SWTTiers t = null;
				t = (SWTTiers) mapper.map(o, SWTTiers.class);
				lswt.add(t);
			}
	...
	public void bind(PaFournisseurSWT paCommercialSWT) {
		try {
			// modelCommercial = new ModelGeneral<SWTTiers>(ibTaTable.getFIBQuery(),classModel);
...
//			tableViewer.setInput(writableList);
	...
	private void changementDeSelection() {
		if(selectedFournisseurArticle!=null && selectedFournisseurArticle.getValue()!=null) {
			if(((SWTTiers) selectedFournisseurArticle.getValue()).getIdTiers()!=null) {
				taTiers = dao.findById(((SWTTiers) selectedFournisseurArticle.getValue()).getIdTiers());
			}
...
//					dao.lgrLocateID(((ResultAffiche) evt.getRetour()).getIdResult());
	...
	protected void actInserer() throws Exception {
		try {
			...
			swtFournisseurArticle = new SWTTiers();			
...
//				setMasterEntity(masterDAO.findById(masterEntity.getIdTiers()));
			for (TaTiers p : masterEntity.getTaFournisseurs()) {
				if(p.getIdTiers()==((SWTTiers) selectedFournisseurArticle.getValue()).getIdTiers()) {
					taTiers = p;
...
//						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTTiers,TaTiersDAO,TaTiers>(dao.rechercheParType("F"),SWTTiers.class,dao.getEntityManager()));
...
//					}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "FOURNISSEUR_ARTICLE";
		try {
			IStatus s = null;
	...
	protected void actEnregistrer() throws Exception {
		...
		try {
			...
			if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
				if( ((SWTTiers) selectedFournisseurArticle.getValue()).getIdTiers()!= taTiers.getIdTiers()) {
					...
					if(trouve) {
						...
						mapperModelToUI.map(taTiers,(SWTTiers) selectedFournisseurArticle.getValue());
					}
					tmp = null;
				}

			} else if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)) {
				//mapperUIToModel.map((SWTTiers) selectedCommercial.getValue(),taCommercial);
...
//			vue.getTfCOMMERCIAL().setEditable(!isUtilise());
	...
	public boolean isUtilise(){
		return 
//		((SWTTiers)selectedFamilleTiers.getValue()).getIdFamille()!=null  
//		&&!dao.recordModifiable(dao.getNomTable(),
//				((SWTTiers)selectedFamilleTiers.getValue()).getIdFamille()))||
				!masterDAO.autoriseModification(masterEntity);		
	}

	public SWTTiers getSwtOldFournisseurArticle() {
		return swtOldFournisseurArticle;
	}

	public void setSwtOldTypeCivilite(SWTTiers swtOldCommercial) {
		this.swtOldFournisseurArticle = swtOldCommercial;
	}

	public void setSwtOldCommercial() {
		if (selectedFournisseurArticle.getValue() != null)
			this.swtOldFournisseurArticle = SWTTiers.copy((SWTTiers) selectedFournisseurArticle.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
			this.swtOldFournisseurArticle = SWTTiers.copy((SWTTiers) selectedFournisseurArticle.getValue());
			tableViewer.setSelection(new StructuredSelection(
					(SWTTiers) selectedFournisseurArticle.getValue()), true);
		}}
	}
	...
	protected void actRefresh() throws Exception {
		...
		if (taTiers!=null) { //enregistrement en cours de modification/insertion
			...
			if(((SWTTiers) selectedFournisseurArticle.getValue()).getIdTiers()!=null)
				idActuel = ((SWTTiers) selectedFournisseurArticle.getValue()).getIdTiers();
		}
	...
	public ModelGeneralObjet<SWTTiers, TaTiersDAO, TaTiers> getModelFournisseurArticle() {

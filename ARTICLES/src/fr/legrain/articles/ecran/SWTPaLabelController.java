...
import fr.legrain.articles.editor.EditorCategorieArticle;
import fr.legrain.articles.editor.EditorFamille;
import fr.legrain.articles.editor.EditorInputCategorieArticle;
import fr.legrain.articles.editor.EditorInputFamille;
import fr.legrain.articles.editor.EditorInputLabelArticle;
import fr.legrain.articles.editor.EditorInputTypeLabelArticle;
import fr.legrain.articles.editor.EditorLabelArticle;
...
import fr.legrain.gestCom.Module_Articles.SWTTypeLabelArticle;
...
public class SWTPaLabelController extends JPABaseControllerSWTStandard
	...
	private SWTTypeLabelArticle swtTypeLabelArticle;
	private SWTTypeLabelArticle swtOldTypeLabelArticle;
	private Realm realm;
	private DataBindingContext dbc;

	private Class classModel = SWTTypeLabelArticle.class;
	private ModelGeneralObjet<SWTTypeLabelArticle,TaLabelArticleDAO,TaLabelArticle> modelTypeLabelArticle = null;
	...
	private LgrDozerMapper<SWTTypeLabelArticle,TaLabelArticle> mapperUIToModel  = new LgrDozerMapper<SWTTypeLabelArticle,TaLabelArticle>();
	private LgrDozerMapper<TaLabelArticle,SWTTypeLabelArticle> mapperModelToUI  = new LgrDozerMapper<TaLabelArticle,SWTTypeLabelArticle>();
	...
	public SWTPaLabelController(PaLabelSWT vue,EntityManager em) {
		...
		dao = new TaLabelArticleDAO(getEm());
		modelTypeLabelArticle = new ModelGeneralObjet<SWTTypeLabelArticle,TaLabelArticleDAO,TaLabelArticle>(dao,classModel);
		setVue(vue);
	 ...
	 * @return
	 */
	public List<SWTTypeLabelArticle> IHMmodel() {
		LinkedList<TaLabelArticle> ldao = new LinkedList<TaLabelArticle>();
		LinkedList<SWTTypeLabelArticle> lswt = new LinkedList<SWTTypeLabelArticle>();

		if(masterEntity!=null && !masterEntity.getTaLabelArticles().isEmpty()) {

			ldao.addAll(masterEntity.getTaLabelArticles());

			LgrDozerMapper<TaLabelArticle,SWTTypeLabelArticle> mapper = new LgrDozerMapper<TaLabelArticle,SWTTypeLabelArticle>();
			for (TaLabelArticle o : ldao) {
				SWTTypeLabelArticle t = null;
				t = (SWTTypeLabelArticle) mapper.map(o, SWTTypeLabelArticle.class);
				lswt.add(t);
			}
	...
	public void bind(PaLabelSWT paCommercialSWT) {
		try {
			// modelCommercial = new ModelGeneral<SWTTypeLabelArticle>(ibTaTable.getFIBQuery(),classModel);
...
//			List<SWTTypeLabelArticle> lswt = IHMmodel();
...
//			tableViewer.setInput(writableList);
	...
	private void changementDeSelection() {
		if(selectedTypeLabelArticle!=null && selectedTypeLabelArticle.getValue()!=null) {
			if(((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue()).getIdLabelArticle()!=null) {
				taLabelArticle = dao.findById(((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue()).getIdLabelArticle());
			}
...
//					dao.lgrLocateID(((ResultAffiche) evt.getRetour()).getIdResult());
	...
	protected void actInserer() throws Exception {
		try {
			...
			swtTypeLabelArticle = new SWTTypeLabelArticle();			
...
//				setMasterEntity(masterDAO.findById(masterEntity.getIdTiers()));
			for (TaLabelArticle p : masterEntity.getTaLabelArticles()) {
				if(p.getIdLabelArticle()==((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue()).getIdLabelArticle()) {
					taLabelArticle = p;
		...
		try {
			...
			case C_MO_EDITION:
				if (silencieu || (!silencieu && MessageDialog.openQuestion(vue.getShell(), MessagesEcran
					...
					List<SWTTypeLabelArticle> l = IHMmodel();
					if(rang!=-1)
...
//					}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "LABEL_ARTICLE";
		try {
			IStatus s = null;
	...
	protected void actEnregistrer() throws Exception {
		...
		try {
			...
			if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
				if( ((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue()).getIdLabelArticle()!= taLabelArticle.getIdLabelArticle()) {
					...
					if(trouve) {
						...
						mapperModelToUI.map(taLabelArticle,(SWTTypeLabelArticle) selectedTypeLabelArticle.getValue());
					}
					tmp = null;
				}

			} else if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_INSERTION) == 0)) {
				//mapperUIToModel.map((SWTTypeLabelArticle) selectedCommercial.getValue(),taCommercial);
//				taCommercial.setTaLabelArticle(masterEntity);
	...
	public TaLabelArticleDAO getDao() {
		return dao;
	}
...
//			vue.getTfCOMMERCIAL().setEditable(!isUtilise());
	...
	public boolean isUtilise(){
		return 
//		((SWTTypeLabelArticle)selectedFamilleTiers.getValue()).getIdFamille()!=null  
//		&&!dao.recordModifiable(dao.getNomTable(),
//				((SWTTypeLabelArticle)selectedFamilleTiers.getValue()).getIdFamille()))||
				!masterDAO.autoriseModification(masterEntity);		
	}

	public SWTTypeLabelArticle getSwtOldTypeLabelArticle() {
		return swtOldTypeLabelArticle;
	}

	public void setSwtOldTypeCivilite(SWTTypeLabelArticle swtOldCommercial) {
		this.swtOldTypeLabelArticle = swtOldCommercial;
	}

	public void setSwtOldCommercial() {
		if (selectedTypeLabelArticle.getValue() != null)
			this.swtOldTypeLabelArticle = SWTTypeLabelArticle.copy((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
			this.swtOldTypeLabelArticle = SWTTypeLabelArticle.copy((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue());
			tableViewer.setSelection(new StructuredSelection(
					(SWTTypeLabelArticle) selectedTypeLabelArticle.getValue()), true);
		}}
	}
	...
	protected void actRefresh() throws Exception {
		...
		if (taLabelArticle!=null) { //enregistrement en cours de modification/insertion
			...
			if(((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue()).getIdLabelArticle()!=null)
				idActuel = ((SWTTypeLabelArticle) selectedTypeLabelArticle.getValue()).getIdLabelArticle();
		}
		...
		if(idActuel!=0) {
			tableViewer.setSelection(new StructuredSelection(recherche(Const.C_ID_LABEL_ARTICLE, idActuel)));
		}else
			tableViewer.selectionGrille(0);	
	}
	...
	public ModelGeneralObjet<SWTTypeLabelArticle, TaLabelArticleDAO, TaLabelArticle> getModelTypeLabelArticle() {

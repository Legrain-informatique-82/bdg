...
public class SWTPaTypeNoteArticleController extends JPABaseControllerSWTStandard
	...
	private TaTNoteArticleDAO dao = null;//new TaTNoteDAO();

	private Object ecranAppelant = null;
	private SWTTNoteArticle swtTypeNote;
	private SWTTNoteArticle swtOldTypeNote;
	private Realm realm;
	...
	private Class classModel = SWTTNoteArticle.class;
	private ModelGeneralObjet<SWTTNoteArticle,TaTNoteArticleDAO,TaTNoteArticle> modelTypeNote = null;//new ModelGeneralObjet<SWTTNote,TaTNoteDAO,TaTNote>(dao,classModel);
	
	private LgrDozerMapper<SWTTNoteArticle,TaTNoteArticle> mapperUIToModel  = new LgrDozerMapper<SWTTNoteArticle,TaTNoteArticle>();
	...
	public SWTPaTypeNoteArticleController(PaTypeNoteArticleSWT vue,EntityManager em) {
		...
		dao = new TaTNoteArticleDAO(getEm());
		modelTypeNote = new ModelGeneralObjet<SWTTNoteArticle,TaTNoteArticleDAO,TaTNoteArticle>(dao,classModel);
		setVue(vue);
...
//			tableViewer.setInput(writableList);
	...
	private void changementDeSelection() {
		if(selectedTypeNote!=null && selectedTypeNote.getValue()!=null) {
			if(((SWTTNoteArticle) selectedTypeNote.getValue()).getIdTNoteArticle()!=null) {
				taTNote = dao.findById(((SWTTNoteArticle) selectedTypeNote.getValue()).getIdTNoteArticle());
			}
	...
	protected void actInserer() throws Exception {
		try {
			if(dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_CONSULTATION)==0) {
				VerrouInterface.setVerrouille(true);
				setSwtOldTypeNote();
				swtTypeNote = new SWTTNoteArticle();
		...
		try {	
			if (selectedTypeNote.getValue()!=null){
				...
				if(!containsEntity(taTNote)) 
					modelTypeNote.getListeEntity().add(taTNote);
				changementDeSelection();
				this.swtOldTypeNote=SWTTNoteArticle.copy((SWTTNoteArticle)selectedTypeNote.getValue());
			}
			return true;
		} catch (Exception e) {
			return false;
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			dao.begin(transaction);
				TaTNoteArticle u = dao.findById(((SWTTNoteArticle) selectedTypeNote.getValue()).getIdTNoteArticle());
				dao.supprimer(u);				
		...
		try {
			...
			case C_MO_INSERTION:
				if (silencieu || (!silencieu && MessageDialog.openQuestion(vue.getShell(), MessagesEcran
					...
					if(((SWTTNoteArticle) selectedTypeNote.getValue()).getIdTNoteArticle()==null){
					modelTypeNote.getListeObjet().remove(((SWTTNoteArticle) selectedTypeNote.getValue()));
					writableList = new WritableList(realm, modelTypeNote.getListeObjet(), classModel);
					tableViewer.setInput(writableList);
					tableViewer.refresh();
					tableViewer.selectionGrille(0);
	...
	protected void actImprimer() throws Exception {
		...
		fonctionGetInfosXmlAndProperties.findInfosFileXml(TaTNoteArticle.class.getName(),SWTTNoteArticle.class.getName(),
				listeChamp,"mapping");
...
//			impression.imprimer(true,pathFileReportDynamic,null,"Type Web",TaTNote.class.getSimpleName(),false);
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "T_NOTE_ARTICLE";
		try {
			IStatus s = null;
			...
			int change=0;
				...
				if(((SWTTNoteArticle) selectedTypeNote.getValue()).getIdTNoteArticle()!=null) {
					u.setIdTNoteArticle(((SWTTNoteArticle) selectedTypeNote.getValue()).getIdTNoteArticle());
				}
	...
	protected void actEnregistrer() throws Exception {
		...
		try {
			...
			if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)||
				...
				LgrDozerMapper<SWTTNoteArticle,TaTNoteArticle> mapper = new LgrDozerMapper<SWTTNoteArticle,TaTNoteArticle>();
				mapper.map((SWTTNoteArticle) selectedTypeNote.getValue(),taTNote);
...
//					modelTypeWeb.getListeEntity().add(TaTNote);
	...
	public boolean isUtilise(){
		return (((SWTTNoteArticle)selectedTypeNote.getValue()).getIdTNoteArticle()!=null &&
		!dao.recordModifiable(dao.getNomTable(),
				((SWTTNoteArticle)selectedTypeNote.getValue()).getIdTNoteArticle()))||
				!dao.autoriseModification(taTNote);		
	}
	
	public SWTTNoteArticle getSwtOldTypeNote() {
		return swtOldTypeNote;
	}

	public void setSwtOldTypeNote(SWTTNoteArticle swtOldTypeNote) {
		...
		if (selectedTypeNote.getValue() != null)
			...
			if(tableViewer.selectionGrille(0)){
				...
				tableViewer.setSelection(new StructuredSelection(
						(SWTTNoteArticle) selectedTypeNote.getValue()), true);
			}
		}
	}
	...
	protected void actRefresh() throws Exception {
		...
		if(u != null) {
			Iterator<SWTTNoteArticle> ite = modelTypeNote.getListeObjet().iterator();
			SWTTNoteArticle tmp = null;
			int i = 0;
			boolean trouve = false;
			while (ite.hasNext() && !trouve) {
				tmp = ite.next();
				if(tmp.getIdTNoteArticle()==u.getIdTNoteArticle()) {
	...
	public ModelGeneralObjet<SWTTNoteArticle,TaTNoteArticleDAO,TaTNoteArticle> getModelTypeNote() {
		return modelTypeNote;
	}

	public TaTNoteArticleDAO getDao() {
		return dao;
	}

	public void setDao(TaTNoteArticleDAO dao) {
		this.dao = dao;
	}

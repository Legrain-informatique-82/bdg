...
import fr.legrain.documents.dao.TaInfosDevisDAO;
...
import fr.legrain.gestCom.Module_Document.IHMEnteteDevis;
...
public class PaEditorDevisController extends JPABaseControllerSWTStandard
...
//	private List<TaDevis> listeDocument = null;
	...
	private TaDevisDAO dao = null;//new TaDevisDAO();

	private Object ecranAppelant = null;
	private IHMEnteteDevis ihmOldEnteteDevis;
	private Realm realm;
	private DataBindingContext dbc;
	private Object selectedDevis = new IHMEnteteDevis();
	private String nomClassController = this.getClass().getSimpleName();
	private Class classModel = IHMEnteteDevis.class;
	...
	private ModelGeneralObjet<IHMEnteteDevis,TaDevisDAO,TaDevis> modelEnteteDevis /*= new ModelGeneralObjet<IHMEnteteDevis,TaDevisDAO,TaDevis>(dao,classModel)*/;
	...
	private TaInfosDevisDAO daoInfosDevis = null;//new TaInfosDevisDAO();
	...
	private LgrDozerMapper<TaInfosDevis,IHMInfosCPaiement> mapperModelToUIInfosDocVersCPaiement = new LgrDozerMapper<TaInfosDevis,IHMInfosCPaiement>();
	private LgrDozerMapper<TaInfosDevis,IHMAdresseInfosFacturation> mapperModelToUIInfosDocVersAdresseFact = new LgrDozerMapper<TaInfosDevis,IHMAdresseInfosFacturation>();
	private LgrDozerMapper<TaInfosDevis,IHMAdresseInfosLivraison> mapperModelToUIInfosDocVersAdresseLiv = new LgrDozerMapper<TaInfosDevis,IHMAdresseInfosLivraison>();
	...
	private LgrDozerMapper<IHMEnteteDevis,TaDevis> mapperUIToModel = new LgrDozerMapper<IHMEnteteDevis,TaDevis>();
	private LgrDozerMapper<TaDevis,IHMEnteteDevis> mapperModelToUI = new LgrDozerMapper<TaDevis,IHMEnteteDevis>();
	
	private TaDevis taDocument = null;
	private TaInfosDevis taInfosDocument = null;
	
	private Menu popupMenuFormulaire = null;
	private Menu popupMenuGrille = null;
	...
	public PaEditorDevisController(PaEditorDevis vue,EntityManager em) {
		...
		daoInfosDevis = new TaInfosDevisDAO(getEm());
...
//						ctrlUnChampsSWT(((PaEditorDevis) getVue()).getTfLIBELLE_DEVIS());
	...
	public void bind(PaEditorDevis PaEditorDevisSWT) {
		try {
			modelEnteteDevis = new ModelGeneralObjet<IHMEnteteDevis,TaDevisDAO,TaDevis>(dao,classModel);
			realm = SWTObservables.getRealm(vue.getParent().getDisplay());

//			modelEnteteFacture.remplirListe();
			if (modelEnteteDevis.getListeObjet().isEmpty()) {
				modelEnteteDevis.getListeObjet().add(new IHMEnteteDevis());
			}
...
//				.getItem(3).getControl()).getBtnReinitialiser(),C_COMMAND_DOCUMENT_REINIT_INFOSTIERS_ID);
	...
	public boolean onClose() throws ExceptLgr {
		...
		try{
			if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
				...
				case C_MO_INSERTION:
				case C_MO_EDITION:
					boolean demande=!((IHMEnteteDevis) selectedDevis).DevisEstVide(taDocument.dateDansExercice(new Date()));
				...
				if (getFocusCourantSWT() instanceof Text) {
					try {
						...
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_TIERS())){
							...
							((IHMEnteteDevis)selectedDevis).setLibelleDocument("Devis N°"+taDocument.getCodeDocument()+" - "+nomTiers);
							
							if(vue.getCbTTC().isEnabled()){
								taDocument.setTtc(u.getTtcTiers());
								((IHMEnteteDevis)selectedDevis).setTtc(LibConversion.intToBoolean(taDocument.getTtc()));
							}
						...
						if(getFocusAvantAideSWT().equals(vue.getTfCODE_DOCUMENT())){
							TaDevis u = null;
							TaDevisDAO t = new TaDevisDAO(getEm());
							u = t.findById(Integer.parseInt(((ResultAffiche) evt.getRetour()).getIdResult()));
							t = null;
							taDocument=u;
	...
	public void calculDateEcheance(Boolean appliquer) {
		...
		if(((IHMEnteteDevis)selectedDevis).getIdDocument()==0|| appliquer) {
			...
			((IHMEnteteDevis)selectedDevis).setDateEchDocument(taDocument.calculDateEcheance(report, finDeMois));
		}
		}
	}
	...
	private boolean changementTiers(boolean refresh) {
		...
		try {						
			...
			////////////////////////////////////////////////////
			if(((IHMEnteteDevis)selectedDevis).getIdDocument()!=0 && 
				...
				if(!idTiersDocumentOriginal.equals(((IHMEnteteDevis)selectedDevis).getIdTiers())){
	...
	public void initialisationDesInfosComplementaires(Boolean recharger,String typeARecharger){
		 ...
		 * - le reste des adresses du tiers
		 */
		if(((IHMEnteteDevis)selectedDevis).getCodeDocument()!=null)
			taInfosDocument = daoInfosDevis.findByCodeDevis(((IHMEnteteDevis)selectedDevis).getCodeDocument());
		else
...
//					dao.getEntityManager().clear();
					...
					mapperModelToUI.map(taDocument, (IHMEnteteDevis) selectedDevis);
					((IHMEnteteDevis) selectedDevis).setCodeDocument(dao.genereCode());
					validateUIField(Const.C_CODE_DOCUMENT,((IHMEnteteDevis) selectedDevis).getCodeDocument());//permet de verrouiller le code genere
					((IHMEnteteDevis) selectedDevis).setCommentaire(DevisPlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.COMMENTAIRE));

					validateUIField(Const.C_CODE_DOCUMENT,((IHMEnteteDevis) selectedDevis).getCodeDocument());
					taDocument.setCodeDocument(((IHMEnteteDevis) selectedDevis).getCodeDocument());
					changementTiers(true);
					...
					if (LibDate.compareTo(date, taInfoEntreprise.getDatedebInfoEntreprise()) == -1) {
						((IHMEnteteDevis) selectedDevis).setDateDocument(taInfoEntreprise.getDatedebInfoEntreprise());
						((IHMEnteteDevis) selectedDevis).setDateEchDocument(LibDate.incrementDate(taInfoEntreprise.getDatedebInfoEntreprise(), 0, 1, 0) );
						((IHMEnteteDevis) selectedDevis).setDateLivDocument(taInfoEntreprise.getDatedebInfoEntreprise());
					} else
						// si date supérieur à dateFin dossier
						if (LibDate.compareTo(date, taInfoEntreprise.getDatefinInfoEntreprise()) == 1) {
							((IHMEnteteDevis) selectedDevis).setDateDocument(taInfoEntreprise.getDatefinInfoEntreprise());
							((IHMEnteteDevis) selectedDevis).setDateEchDocument(LibDate.incrementDate(taInfoEntreprise.getDatefinInfoEntreprise(), 0, 1, 0) );
							((IHMEnteteDevis) selectedDevis).setDateLivDocument(taInfoEntreprise.getDatefinInfoEntreprise());
						} else {
							((IHMEnteteDevis) selectedDevis).setDateDocument(new Date());
							((IHMEnteteDevis) selectedDevis).setDateEchDocument(LibDate.incrementDate(new Date(), 0, 1, 0) );
							((IHMEnteteDevis) selectedDevis).setDateLivDocument(new Date());
						}
					((IHMEnteteDevis) selectedDevis).setCodeTiers("");
					((IHMEnteteDevis) selectedDevis).setIdAdresse(0);
					((IHMEnteteDevis) selectedDevis).setIdAdresseLiv(0);
					...
					if(typePaiementDefaut!=null){
						 ...
						 */
						if(taTPaiement!=null) {
							((IHMEnteteDevis) selectedDevis).setCodeTPaiement(typePaiementDefaut);
							taDocument.setTaTPaiement(taTPaiement);
						}
					}					
					mapperUIToModel.map(((IHMEnteteDevis) selectedDevis), taDocument);
...
//					setIhmOldEnteteDevis();
//					taDocument = dao.findById(((IHMEnteteDevis) selectedDevis).getIdDocument());
...
//				}else 
	...
	public Boolean setIhmOldEnteteDocumentRefresh(){
		try {			
			if (selectedDevis != null){
				this.ihmOldEnteteDevis = IHMEnteteDevis.copy((IHMEnteteDevis) selectedDevis);
				if (taDocument!=null){
					//dao.getEntityManager().clear();
	...
	protected void actSupprimer() throws Exception {
		...
		try {
			...
			if (MessageDialog.openConfirm(vue.getShell(),
				...
				TaDevis u = dao.findById(((IHMEnteteDevis) selectedDevis).getIdDocument());
				dao.supprimer(u);
...
//					listeDocument.remove(taDocument);
	...
	protected void actAnnuler(boolean annulationForcee,boolean insertion,boolean messageAffiche) throws Exception {
		...
		if (!vue.isDisposed() && !vue.getShell().isDisposed()) {
			...
			boolean message = messageAffiche && !((IHMEnteteDevis) selectedDevis).DevisEstVide(taDocument.dateDansExercice(new Date()));
					...
					case C_MO_EDITION:
						...
						case C_MO_EDITION:
							...
							if (repondu) {
								...
								dao.annulerCodeGenere(((IHMEnteteDevis) selectedDevis).getCodeDocument(),Const.C_CODE_DOCUMENT);
...
//				true,ConstEdition.FICHE_FILE_REPORT_DEVIS,namePlugin,TaDevis.class.getSimpleName());
	...
	protected boolean aideDisponible() {
		...
		switch ((PaEditorDevisController.this.dao.getModeObjet().getMode())) {
		case C_MO_CONSULTATION:
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				case C_MO_CONSULTATION:
					if (getFocusCourantSWT().equals(vue.getTfCODE_DOCUMENT())) {
						...
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<IHMAideDevis,TaDevisDAO,TaDevis>(dao,IHMAideDevis.class));
...
//							retourPage=ChangementDePageEvent.DEBUT;
	...
	public IHMEnteteDevis getIhmOldEnteteDevis() {
		return ihmOldEnteteDevis;
	}

	public void setIhmOldEnteteDevis(IHMEnteteDevis ihmOldEnteteDevis) {
		this.ihmOldEnteteDevis = ihmOldEnteteDevis;
	}

	public void setIhmOldEnteteDevis() {
		if (selectedDevis != null)
			this.ihmOldEnteteDevis = IHMEnteteDevis
					.copy((IHMEnteteDevis) selectedDevis);
		else {
		}
	}
...
//			}
	...
	protected void actRefresh() throws Exception {
		try{
			vue.setCursor(Display.getCurrent().getSystemCursor(SWT.CURSOR_WAIT));
		if (dao.dataSetEnModif()) {
			if (taDocument!=null && selectedDevis!=null && (IHMEnteteDevis) selectedDevis!=null) {
				mapperModelToUI.map(taDocument, (IHMEnteteDevis) selectedDevis);
			}
		}
		listeDocument=null;
	...
	private Boolean remonterDocument(String codeDocument) throws Exception {
		try {
			...
			if(taDocument!=null) {
				factureVide = ((IHMEnteteDevis) selectedDevis).DevisEstVide(taDocument.dateDansExercice(new Date()));
			}
...
//				.isVerrouille();
//		String codeTmp = ((IHMEnteteDevis) selectedDevis)
//				.getCodeDevis();
//		try {
...
//
//				((IHMEnteteDevis) selectedDevis)
//						.setIHMEnteteDevis(((SWTEnteteDevis) swtDevis
//								.getEntete()));
//
//				// récupérer code validé dans facture, dataset et interface
...
//	}
	public String[][] initAdapterDocument() {
		...
		if(affichageCtrlEspace){
		if(listeDocument==null){
			TaDevisDAO taDocDAO = new TaDevisDAO(getEm());
			TaInfoEntrepriseDAO taInfoEntrepriseDAO = new TaInfoEntrepriseDAO(getEm());
			TaInfoEntreprise taInfoEntreprise = taInfoEntrepriseDAO.findInstance();
...
//			if(newValue!=null){
//				if((((IHMEnteteDevis)selectedDevis).getDateLivDocument()!=null && 
//						((IHMEnteteDevis)selectedDevis).getDateLivDocument().before(newValue))||
//						((IHMEnteteDevis)selectedDevis).getDateLivDocument()==null ){
//					taDocument.setDateLivDocument(newValue);
//					((IHMEnteteDevis)selectedDevis).setDateLivDocument(newValue);
//					}
//			}
//		}
...
//		}
	...
	public Date dateDansPeriodeEch(Date newValue,String champ) throws Exception{
		if(champ.equals(Const.C_DATE_DOCUMENT)){
			...
			newValue=taDocument.dateDansExercice(newValue);
			if(newValue!=null){
				if((((IHMEnteteDevis)selectedDevis).getDateEchDocument()!=null && 
						((IHMEnteteDevis)selectedDevis).getDateEchDocument().before(newValue))||
						((IHMEnteteDevis)selectedDevis).getDateEchDocument()==null ){					
					taDocument.setDateEchDocument(LibDate.incrementDate(newValue, 0, 1, 0));
					((IHMEnteteDevis)selectedDevis).setDateEchDocument(LibDate.incrementDate(newValue, 0, 1, 0));
					}else{
						...
						((IHMEnteteDevis)selectedDevis).setDateEchDocument(newValueEch);
						}
					}
			}
...
//	}
	...
	public TaDevisDAO getDao() {
		return dao;
	}

	public void setDao(TaDevisDAO dao) {
		this.dao = dao;
	}
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "DEVIS";
		try {
			IStatus s = null;
...
//				}
				...
				if(((IHMEnteteDevis) selectedDevis).getIdDocument()!=null) {
					u.setIdDocument(((IHMEnteteDevis) selectedDevis).getIdDocument());
				}
				if(nomChamp.equals(Const.C_CODE_DOCUMENT)){
					...
					value=taDocument.dateDansExercice((Date)value);
					PropertyUtils.setSimpleProperty(u, nomChamp,value );
...
//					value=(dateDansPeriodeLiv((Date)value,nomChamp));
			...
			if(s.getSeverity()!=Status.ERROR){
				if(nomChamp.equals(Const.C_DATE_DOCUMENT)){
					...
					((IHMEnteteDevis)selectedDevis).setDateDocument((Date)value);
					PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_DOCUMENT, value);
					if(changement){
						((IHMEnteteDevis)selectedDevis).setDateEchDocument(LibDate.incrementDate(taDocument.getDateDocument(), 0, 1, 0));
						PropertyUtils.setSimpleProperty(taDocument, Const.C_DATE_ECH_DOCUMENT, ((IHMEnteteDevis)selectedDevis).getDateEchDocument());
						((IHMEnteteDevis)selectedDevis).setDateLivDocument(taDocument.getDateDocument());
				...
				if(nomChamp.equals(Const.C_DATE_LIV_DOCUMENT)) {
					((IHMEnteteDevis)selectedDevis).setDateLivDocument((Date)value);

...
import fr.legrain.documents.dao.TaLDevisDAO;
...
import fr.legrain.gestCom.Module_Document.IHMLDevis;
...
public class PaLigneDevisController extends JPABaseControllerSWTStandard
implements RetourEcranListener, SWTModificationDocumentListener, ISelectionProvider {
	...
	private TaLDevisDAO dao = null;//new TaLDevisDAO();
	
	private Object ecranAppelant = null;
	private IHMLDevis ihmLDevis;
	private IHMTotauxLignesDocument ihmTotauxLignesDocument;
	private IHMLDevis ihmOldLDevis;
	private Realm realm;
	private DataBindingContext dbc;
	private DataBindingContext dbcTotaux = null;

//	private ModelLigneDevis modelLDevis /*=  new ModelLigneFacture(ibTaTable)*/;
	private Class classModel = IHMLDevis.class;
	private ModelGeneralObjet<IHMLDevis,TaLDevisDAO,TaLDevis> modelLDocument /*= new ModelGeneralObjet<IHMLDevis,TaLDevisDAO,TaLDevis>(dao,classModel)*/;
	...
	private TaDevisDAO masterDAO = null;
	
	String[] listeCodeArticles=null;
	String[] listeDescriptionArticles=null;

	private LgrDozerMapper<IHMLDevis,TaLDevis> mapperUIToModel = new LgrDozerMapper<IHMLDevis,TaLDevis>();
	private LgrDozerMapper<TaLDevis,IHMLDevis> mapperModelToUI = new LgrDozerMapper<TaLDevis,IHMLDevis>();
...
//			// fenetre sur ESC
	 ...
	 * @return
	 */
	public List<IHMLDevis> IHMmodel() {
		LinkedList<TaLDevis> ldao = new LinkedList<TaLDevis>();
		LinkedList<IHMLDevis> lswt = new LinkedList<IHMLDevis>();
		
		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {

			ldao.addAll(masterEntity.getLignes());

			LgrDozerMapper<TaLDevis,IHMLDevis> mapper = new LgrDozerMapper<TaLDevis,IHMLDevis>();
			for (TaLDevis o : ldao) {
				IHMLDevis t = null;
				t = (IHMLDevis) mapper.map(o, IHMLDevis.class);
				lswt.add(t);
			}
	...
	public void bind(PaSaisieLigneDevis paArticleSWT) {
		try {
			if (modelLDocument==null)modelLDocument = new ModelGeneralObjet<IHMLDevis, TaLDevisDAO, TaLDevis>(dao,classModel);
...
//			}
			...
			articleContentProposal.addContentProposalListener(new IContentProposalListener2(){
				...
				public void proposalPopupOpened(ContentProposalAdapter adapter) {
					...
					if(!dao.dataSetEnModif()&& taLDevis.ligneEstVide()  )
...
//	);
	...
	protected void actChangeTva() throws Exception{
		if(!LibChaine.empty(((IHMLDevis)selectedLigneDevis.getValue()).getCodeArticle())
			...
			InputDialog input = new InputDialog(vue.getShell()
					...
					((IHMLDevis)selectedLigneDevis.getValue()).getCodeTvaLDocument(),new IInputValidator() {
			...
			if(masterDAO.dataSetEnModif()){
				...
				if(input.getValue()!=null ){
					if(MessageDialog.openConfirm(vue.getShell()
						...
						((IHMLDevis)selectedLigneDevis.getValue()).setCodeTvaLDocument(retour);
...
//			}
			if(continuer){
				...
				for (TaLDevis p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument().equals(((IHMLDevis) selectedLigneDevis.getValue()).getNumLigneLDocument())) {
						taLDevis = p;
						taLDevis.setLegrain(true);
						ihmLDevis= IHMLDevis.copy((IHMLDevis) selectedLigneDevis.getValue());
					}				
				}
				initEtatBouton(false);
			}
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				switch ((PaLigneDevisController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:
...
//						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTArticle,TaArticleDAO,TaArticle>(SWTArticle.class,getEm()));
					...
					if (getFocusCourantSWT()==vue.getTfPRIX_U_L_DEVIS()){
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTPrix,TaPrixDAO,TaPrix>(taLDevis.getTaArticle().getTaPrixes(),SWTPrix.class,getEm()));
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		String validationContext = "L_DEVIS";
		try {
			IStatus s = null;
			boolean change=true;
			if(nomChamp.equals(Const.C_CODE_ARTICLE)) {
				if(value=="")return new org.eclipse.core.runtime.Status(org.eclipse.core.runtime.Status.OK,gestComBdPlugin.PLUGIN_ID,"OK"); 
					...
					if(change){
						taLDevis.setTaArticle(f);
						taLDevis.initQte2(f);
						mapperModelToUI.map(taLDevis, ihmLDevis);
					}
					}
					dao = null;
			...
			if(s.getSeverity()!=IStatus.ERROR) {
				...
				mapperModelToUI.map(taLDevis, ((IHMLDevis)selectedLigneDevis.getValue()));
			}
	...
	protected void actEnregistrer(boolean insertion) throws Exception {
		try {
			if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
				...
				ctrlTousLesChampsAvantEnregistrementSWT();
				if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
					LgrDozerMapper<IHMLDevis,TaLDevis> mapper = new LgrDozerMapper<IHMLDevis,TaLDevis>();
					mapper.map((IHMLDevis) selectedLigneDevis.getValue(),taLDevis);
...
//	}
	...
	public boolean isUtilise(){
		return ((IHMLDevis)selectedLigneDevis.getValue()).getIdLDocument()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((IHMLDevis)selectedLigneDevis.getValue()).getIdLDocument());		
	}
	public IHMLDevis getIhmOldLDevis() {
		return ihmOldLDevis;
	}

	public void setIhmOldLDevis(IHMLDevis swtOldDevis) {
		this.ihmOldLDevis = swtOldDevis;
	}

	public void setIhmOldLDevis() {
		if (selectedLigneDevis!=null && selectedLigneDevis.getValue() != null)
			this.ihmOldLDevis = IHMLDevis.copy((IHMLDevis) selectedLigneDevis.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
				this.ihmOldLDevis = IHMLDevis.copy((IHMLDevis) selectedLigneDevis.getValue());
				tableViewer.setSelection(new StructuredSelection(
						(IHMLDevis) selectedLigneDevis.getValue()), true);
			}}
	}
...
//
//			for (IHMLDevis ihmLigne : modelLDevis.getListeObjet()) {
//				SWTLigneDevis swtLigne = new SWTLigneDevis((IHMLDevis)selectedLigneDevis.getValue());
...
//			}
	...
	private class SelectionGrille extends SelectionAdapter {
		public void widgetSelected(SelectionEvent e) {
			...
			if(selectedLigneDevis.getValue()!=null && masterEntity.getLignes().size()>vue.getGrille().getSelectionIndex()){
				for (TaLDevis p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument()==((IHMLDevis) selectedLigneDevis.getValue()).getNumLigneLDocument()) {
						taLDevis = p;
					}				
				}
			}
	...
	public IHMLDevis getSelectedLigneDevis() {
		IHMLDevis resultat = null;
		if(selectedLigneDevis!=null && selectedLigneDevis.getValue()!=null)
			resultat = (IHMLDevis)selectedLigneDevis.getValue();
		return resultat;
	};	
	...
	protected void InsertionLigne(Boolean ajout, boolean initFocus) throws Exception{
		...
		try {
			...
			if(masterDAO.dataSetEnModif()){
			setIhmOldLDevis();
			ihmLDevis = new IHMLDevis();
			dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_INSERTION);
			taLDevis = new TaLDevis(true); 
			taLDevis.setTaTLigne(new TaTLigneDAO(getEm()).findByCode(Const.C_CODE_T_LIGNE_C));
			
			List<IHMLDevis> modelListeLigneDevis = IHMmodel();
...
//			modelListeLigneDevis = IHMmodel();
			...
			ihmLDevis.setNumLigneLDocument(taLDevis.getNumLigneLDocument());
			...
			taLDevis.setNumLigneLDocument(ihmLDevis.getNumLigneLDocument());
...
//								e.doit=false;
			...
			public boolean changementPageValide(){
				if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
					...
					if(taLDevis!=null && selectedLigneDevis!=null && ((IHMLDevis) selectedLigneDevis.getValue())!=null) {
						mapperUIToModel.map((IHMLDevis) selectedLigneDevis.getValue(),taLDevis);
					}
				}
				return true;
			}
			...
			public TaDevisDAO getMasterDAO() {
				return masterDAO;
			}

			public void setMasterDAO(TaDevisDAO masterDAO) {

...
public class SWTPaFamilleTiersController extends JPABaseControllerSWTStandard
...
//	private SWT_IB_TA_R_COMMERCIAL dao = new SWT_IB_TA_R_COMMERCIAL();
	private TaFamilleTiersDAO dao = null;//new TaFamilleTiersDAO();
	...
	private SWTFamilleTiers swtFamilleTiers;
	private SWTFamilleTiers swtOldFamilleTiers;
	private Realm realm;
	private DataBindingContext dbc;

	private Class classModel = SWTFamilleTiers.class;
//	private ModelGeneral<SWTFamilleTiers> modelCommercial = new ModelGeneral<SWTFamilleTiers>(dao.getFIBQuery(),classModel);
	private ModelGeneralObjet<SWTFamilleTiers,TaFamilleTiersDAO,TaFamilleTiers> modelFamilleTiers = null;//new ModelGeneralObjet<SWTFamilleTiers,TaFamilleTiersDAO,TaFamilleTiers>(dao,classModel);
	...
	private LgrDozerMapper<SWTFamilleTiers,TaFamilleTiers> mapperUIToModel  = new LgrDozerMapper<SWTFamilleTiers,TaFamilleTiers>();
	private LgrDozerMapper<TaFamilleTiers,SWTFamilleTiers> mapperModelToUI  = new LgrDozerMapper<TaFamilleTiers,SWTFamilleTiers>();
	...
	public SWTPaFamilleTiersController(PaFamilleSWT vue,EntityManager em) {
		...
		modelFamilleTiers = new ModelGeneralObjet<SWTFamilleTiers,TaFamilleTiersDAO,TaFamilleTiers>(dao,classModel);
		setVue(vue);
	 ...
	 * @return
	 */
	public List<SWTFamilleTiers> IHMmodel() {
		LinkedList<TaFamilleTiers> ldao = new LinkedList<TaFamilleTiers>();
		LinkedList<SWTFamilleTiers> lswt = new LinkedList<SWTFamilleTiers>();

		if(masterEntity!=null && !masterEntity.getTaFamilleTierses().isEmpty()) {

			ldao.addAll(masterEntity.getTaFamilleTierses());

			LgrDozerMapper<TaFamilleTiers,SWTFamilleTiers> mapper = new LgrDozerMapper<TaFamilleTiers,SWTFamilleTiers>();
			for (TaFamilleTiers o : ldao) {
				SWTFamilleTiers t = null;
				t = (SWTFamilleTiers) mapper.map(o, SWTFamilleTiers.class);
				lswt.add(t);
			}
...
//			List<SWTFamilleTiers> lswt = IHMmodel();
...
//			tableViewer.setInput(writableList);
	...
	private void changementDeSelection() {
		if(selectedFamilleTiers!=null && selectedFamilleTiers.getValue()!=null) {
			if(((SWTFamilleTiers) selectedFamilleTiers.getValue()).getIdFamille()!=null) {
				taFamilleTiers = dao.findById(((SWTFamilleTiers) selectedFamilleTiers.getValue()).getIdFamille());
				...
				if(taFamilleTiers!=null 
					...
					((SWTFamilleTiers) selectedFamilleTiers.getValue()).setDefaut(true);
				} else {
					((SWTFamilleTiers) selectedFamilleTiers.getValue()).setDefaut(false);
				}
...
//					dao.lgrLocateID(((ResultAffiche) evt.getRetour()).getIdResult());
	...
	protected void actInserer() throws Exception {
		try {
			...
			swtFamilleTiers = new SWTFamilleTiers();			
...
//				setMasterEntity(masterDAO.findById(masterEntity.getIdTiers()));
			for (TaFamilleTiers p : masterEntity.getTaFamilleTierses()) {
				if(p.getIdFamille()==((SWTFamilleTiers) selectedFamilleTiers.getValue()).getIdFamille()) {
					taFamilleTiers = p;
		...
		try {
			...
			case C_MO_EDITION:
				if (silencieu || (!silencieu && MessageDialog.openQuestion(vue.getShell(), MessagesEcran
					...
					List<SWTFamilleTiers> l = IHMmodel();
					if(rang!=-1)
...
//					}
	...
	protected void actEnregistrer() throws Exception {
		...
		try {
			...
			if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
				if( ((SWTFamilleTiers) selectedFamilleTiers.getValue()).getIdFamille()!= taFamilleTiers.getIdFamille()) {
					...
					if(trouve) {
						...
						mapperModelToUI.map(taFamilleTiers,(SWTFamilleTiers) selectedFamilleTiers.getValue());
					}
					tmp = null;
				}
	...
	public TaFamilleTiersDAO getDao() {
		return dao;
	}
...
//				((SWTFamilleTiers)selectedFamilleTiers.getValue()).getIdFamille()))||
	...
	public SWTFamilleTiers getSwtOldFamilleTiers() {
		return swtOldFamilleTiers;
	}

	public void setSwtOldTypeCivilite(SWTFamilleTiers swtOldCommercial) {
		this.swtOldFamilleTiers = swtOldCommercial;
	}

	public void setSwtOldCommercial() {
		if (selectedFamilleTiers.getValue() != null)
			this.swtOldFamilleTiers = SWTFamilleTiers.copy((SWTFamilleTiers) selectedFamilleTiers.getValue());
		else {
			if(tableViewer.selectionGrille(0)){
			this.swtOldFamilleTiers = SWTFamilleTiers.copy((SWTFamilleTiers) selectedFamilleTiers.getValue());
			tableViewer.setSelection(new StructuredSelection(
					(SWTFamilleTiers) selectedFamilleTiers.getValue()), true);
		}}
	}
	...
	protected void actRefresh() throws Exception {
		...
		} else if(selectedFamilleTiers!=null && (SWTFamilleTiers) selectedFamilleTiers.getValue()!=null) {
			if(((SWTFamilleTiers) selectedFamilleTiers.getValue()).getIdFamille()!=null)
				idActuel = ((SWTFamilleTiers) selectedFamilleTiers.getValue()).getIdFamille();
		}
		...
		if(idActuel!=0) {
			tableViewer.setSelection(new StructuredSelection(recherche(Const.C_ID_FAMILLE, idActuel)));
		}else
			tableViewer.selectionGrille(0);	
	}
	...
	public ModelGeneralObjet<SWTFamilleTiers, TaFamilleTiersDAO, TaFamilleTiers> getModelFamilleTiers() {

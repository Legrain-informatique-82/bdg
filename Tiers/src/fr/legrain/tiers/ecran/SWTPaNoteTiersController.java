...
import fr.legrain.tiers.editor.EditorInputNoteTiers;
import fr.legrain.tiers.editor.EditorInputTypeNoteTiers;
import fr.legrain.tiers.editor.EditorNoteTiers;
import fr.legrain.tiers.editor.EditorTypeNoteTiers;

public class SWTPaNoteTiersController extends JPABaseControllerSWTStandard
...
//						paramAfficheAideRecherche.setChampsIdentifiant(Const.C_ID_NOTE_TIERS);
				...
				case C_MO_INSERTION:
					try {
						if(getFocusCourantSWT().equals(vue.getTfCODE_T_NOTE_TIERS())){
							...
							paramAfficheAideRecherche.setControllerAppelant(getThis());
							paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTTNoteTiers,TaTNoteTiersDAO,TaTNoteTiers>(SWTTNoteTiers.class,dao.getEntityManager()));
	...
	protected void actRefresh() throws Exception {
		...
		if(idActuel!=0) {
			tableViewer.setSelection(new StructuredSelection(recherche(Const.C_ID_NOTE_TIERS, idActuel)));
		}else
			tableViewer.selectionGrille(0);
	}

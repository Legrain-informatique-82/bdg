...
public class PaTypeFamilleTiersController extends JPABaseControllerSWTStandard
implements RetourEcranListener {
	...
	private SWTFamilleTiers swtFamille;
	private SWTFamilleTiers swtOldFamille;
	private Realm realm;
	private DataBindingContext dbc;

	private Class classModel = SWTFamilleTiers.class;
	private ModelGeneralObjet<SWTFamilleTiers,TaFamilleTiersDAO,TaFamilleTiers> modelFamille =
		new ModelGeneralObjet<SWTFamilleTiers,TaFamilleTiersDAO,TaFamilleTiers>(dao,classModel);
...
//			tableViewer.setInput(writableList);
			...
			selectedFamille.addChangeListener(new IChangeListener() {
				...
				public void handleChange(ChangeEvent event) {
					if(selectedFamille!=null && selectedFamille.getValue()!=null) {
						if(((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille()!=null) {
							taFamilleTiers = dao.findById(((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille());
						}
	...
	protected void actInserer() throws Exception {
		try {
			VerrouInterface.setVerrouille(true);
			setSwtOldFamille();
			swtFamille = new SWTFamilleTiers();
...
//			taFamilleTiers = dao.findById(((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille());
	...
	public boolean setSwtOldFamilleRefresh() {
		try {	
			if (selectedFamille.getValue()!=null){
				...
				this.swtOldFamille=SWTFamilleTiers.copy((SWTFamilleTiers)selectedFamille.getValue());
			}
			return true;
		} catch (Exception e) {
			return false;
		...
		try {
			...
			if(isUtilise())MessageDialog.openInformation(vue.getShell(), MessagesEcran
				...
				if (MessageDialog.openConfirm(vue.getShell(), MessagesEcran
					...
					TaFamilleTiers u = dao.findById(((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille());
					dao.supprimer(u);
				...
				if (MessageDialog.openQuestion(vue.getShell(), MessagesEcran
					...
					if(((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille()==null){
						modelFamille.getListeObjet().remove(
								((SWTFamilleTiers) selectedFamille.getValue()));
	...
	protected void actImprimer() throws Exception {
		...
		fonctionGetInfosXmlAndProperties.findInfosFileXml(TaFamilleTiers.class.getName(),SWTFamilleTiers.class.getName(),
				listeChamp,"mapping");
...
//			impression.imprimer(true,pathFileReportDynamic,null,"Famille",TaFamilleTiers.class.getSimpleName(),false);
	...
	public IStatus validateUIField(String nomChamp,Object value) {
		...
		try {
			...
			if(((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille()!=null) {
				u.setIdFamille(((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille());
			}
	...
	protected void actEnregistrer() throws Exception {
		...
		try {
			...
			if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
				...
				mapperUIToModel.map((SWTFamilleTiers) selectedFamille.getValue(),taFamilleTiers);

				dao.enregistrerMerge(taFamilleTiers);
...
//	}
	...
	public boolean isUtilise(){
		return ((SWTFamilleTiers)selectedFamille.getValue()).getIdFamille()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((SWTFamilleTiers)selectedFamille.getValue()).getIdFamille());		
	}
	public SWTFamilleTiers getSwtOldFamille() {
		return swtOldFamille;
	}

	public void setSwtOldFamille(SWTFamilleTiers swtOldFamille) {
		...
		if (selectedFamille.getValue() != null){
			...
			if(tableViewer.selectionGrille(0)){
				...
				tableViewer.setSelection(new StructuredSelection(
						(SWTFamilleTiers) selectedFamille.getValue()), true);
			}}
	}
	...
	protected void actRefresh() throws Exception {
		...
		} else if(selectedFamille!=null && (SWTFamilleTiers) selectedFamille.getValue()!=null) {
			idActuel = ((SWTFamilleTiers) selectedFamille.getValue()).getIdFamille();
		}
	...
	public ModelGeneralObjet<SWTFamilleTiers,TaFamilleTiersDAO,TaFamilleTiers> getModelFamille() {
		return modelFamille;
	}

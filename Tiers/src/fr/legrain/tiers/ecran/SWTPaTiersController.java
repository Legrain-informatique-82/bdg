...
public class SWTPaTiersController extends JPABaseControllerSWTStandard 
...
//			vue.getPaGrille().setMenu(popupMenuGrille);
	...
	public void bindID(PaTiersSWT paTiersSWT, int idTiers ){
		SWTTiers swtTiersTmp = new SWTTiers();
	...
	public void bind(PaTiersSWT paTiersSWT, SWTTiers selection ){
		try {
...
//		}
		if(swtTiers!=null) {
			if(swtTiers.getIdTiers()!=null) {
				taTiers = dao.findById(swtTiers.getIdTiers());
			}
			//null a tester ailleurs, car peut etre null en cas d'insertion
			fireChangementDeSelection(new ChangementDeSelectionEvent(SWTPaTiersController.this));
		}
...
//		mapInitFocus.put(ModeObjet.EnumModeObjet.C_MO_CONSULTATION,vue.getGrille());
		mapInitFocus.put(ModeObjet.EnumModeObjet.C_MO_CONSULTATION,vue.getTfCODE_TIERS());
	...
	protected class HandlerAjoutAdresse extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheAdresse paramAfficheAdresse = new ParamAfficheAdresse();
				paramAfficheAdresse.setIdTiers(LibConversion.integerToString(swtTiers.getIdTiers()));
	...
	protected class HandlerAjoutCommercial extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheCommercial paramAfficheCommercial = new ParamAfficheCommercial();
				paramAfficheCommercial.setIdTiers(LibConversion.integerToString(swtTiers.getIdTiers()));
				((LgrEditorPart)e).setPanel(((LgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutCompl extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheCompl paramAfficheCompl = new ParamAfficheCompl();
				paramAfficheCompl.setIdTiers(swtTiers.getIdTiers());
				((LgrEditorPart)e).setPanel(((LgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutConditionPaiement extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheConditionPaiement paramAfficheConditionPaiement = new ParamAfficheConditionPaiement();
				paramAfficheConditionPaiement.setIdTiers(swtTiers.getIdTiers());
				((LgrEditorPart)e).setPanel(((LgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutEmail extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheEmail paramAfficheEmail = new ParamAfficheEmail();
				paramAfficheEmail.setIdTiers(LibConversion.integerToString(swtTiers.getIdTiers()));
				((LgrEditorPart)e).setPanel(((LgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutSiteWeb extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheWeb paramAfficheWeb = new ParamAfficheWeb();
				paramAfficheWeb.setIdTiers(swtTiers.getIdTiers());
				((LgrEditorPart)e).setPanel(((LgrEditorPart)e).getController().getVue());
	...
	protected class HandlerAjoutTelephone extends LgrAbstractHandler {
		...
		public Object execute(ExecutionEvent event) throws ExecutionException{
			try {
				...
				ParamAfficheTelephone paramAfficheTelephone = new ParamAfficheTelephone();
				paramAfficheTelephone.setIdTiers(swtTiers.getIdTiers());
				((LgrEditorPart)e).setPanel(((LgrEditorPart)e).getController().getVue());
		...
		try {
			...
			if(isUtilise())MessageDialog.openInformation(vue.getShell(), MessagesEcran
				...
				if (MessageDialog.openConfirm(vue.getShell(), MessagesEcran
					...
					TaTiers u = dao.findById(swtTiers.getIdTiers());
					dao.supprimer(u);
...
//					}
				...
				case C_MO_INSERTION:										
					...
					//					}
					if(SWTPaTiersController.this.dao.getModeObjet().getMode()==EnumModeObjet.C_MO_EDITION){
	...
	private void initCodeCompta(){
		if(swtTiers==null){
			swtTiers=new SWTTiers();
			swtTiers = SWTTiers.copy(swtTiers);
		}
//		if(swtTiers!=null){
...
//							&& !value.equals("")
				...
				if(swtTiers!=null && swtTiers.getIdTiers()!=null) {
					u.setIdTiers(swtTiers.getIdTiers());
				}
...
//						actRefresh(); 
						...
						swtTiers.setIdTiers(taTiers.getIdTiers());
...
//						!dao.autoriseModification(taTiers);	
		return (swtTiers!=null &&
				swtTiers.getIdTiers()!=null && !dao.recordModifiable(dao.getNomTable(),swtTiers.getIdTiers()))
				|| !dao.autoriseModification(taTiers);
	}
	...
	public void setSwtOldTiers() {
		if(swtTiers!=null) {
			this.swtOldTiers=SWTTiers.copy(swtTiers);
//		if (selectedTiers!=null&&selectedTiers.getValue()!=null){
...
//				}
	...
	protected void actRefresh() throws Exception {
		...
		} else if(swtTiers!=null && swtTiers.getIdTiers()!=null) {
			idActuel = swtTiers.getIdTiers();
		}

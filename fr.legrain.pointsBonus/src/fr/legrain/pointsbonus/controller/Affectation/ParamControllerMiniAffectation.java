...
public class ParamControllerMiniAffectation extends AbstractControllerMini implements RetourEcranListener {

	static Logger logger = Logger.getLogger(ParamControllerMiniAffectation.class.getName());	
	...
	private FormPageControllerPrincipalAffectation masterController = null;
...
//	public static final String etatEnCoursCode = "ENCOURS";
	...
	private PaFormPageAffectation vue = null;

	private TaInfoEntrepriseDAO taInfoEntrepriseDAO = null;
	private TaInfoEntreprise taInfoEntreprise = null;
	private IObservableValue selectionComplementaire;
	protected List<IHMPeriode> modelPeriode = null;
	
	private Map<String,String> mapTypeFamille = new HashMap<String, String>();
	...
	public ParamControllerMiniAffectation(FormPageControllerPrincipalAffectation masterContoller, PaFormPageAffectation vue, EntityManager em) {
		this.vue = vue;
	...
	public void actRefresh(){
		...
		codeFamille = vue.getCompositeSectionParam().getCbFamille().getText();
		codeArticle = vue.getCompositeSectionParam().getTfCodeArticle().getText();
...
//		}
	...
	public ParamControllerMiniAffectation getThis() {
		return this;
	}
	/* Permet le rafraîchissement des differents composites quand on clique sur le bouton */
	private Action refreshAction = new Action("Rechercher",PluginPointBonus.getImageDescriptor(PaCompositeSectionParamAffectation.iconPath)) { 
		@Override 
		public void run() { 
			datedeb = LibDateTime.getDate(vue.getCompositeSectionParam().getCdateDeb());
...
//						String contributorName = confElements[jj].getContributor().getName();
	...
	protected void initActions() {	
		if(!toolBarInitialise) {
			...
			vue.getCompositeSectionParam().getBtnRefesh().setImage(PluginPointBonus.getImageDescriptor(PaCompositeSectionParamAffectation.iconRefreshPath).createImage());
			...
			vue.getCompositeSectionParam().getBtnAideArticle().setImage(PluginPointBonus.getImageDescriptor(PaCompositeSectionParamAffectation.iconFindPath).createImage());
			vue.getCompositeSectionParam().getBtnAideArticle().addSelectionListener(new SelectionListener() {
			...
			TaFamilleDAO taFamilleDAO = new TaFamilleDAO();
			List<TaFamille> listeFamille = taFamilleDAO.selectAll();
			

			int i = 1;
			int selection = -1;
			for (TaFamille taFamille : listeFamille) {
//				if(taTAbonnement.getTypes()!=null ){
							mapTypeFamille.put(taFamille.getCodeFamille(), taFamille.getLibcFamille());
							vue.getCompositeSectionParam().getCbFamille().add(taFamille.getCodeFamille());
//				}
				i++;
			}
			if(selection==-1)
				selection=0;
			vue.getCompositeSectionParam().getCbFamille().select(selection);
			
			selectionEtat();
			
			vue.getCompositeSectionParam().getCbFamille().addSelectionListener(new SelectionListener() {
...
//			refreshAction.run();
	...
	protected void initMapComposantChamps() {
		...
		vue.getCompositeSectionParam().getCbFamille().addModifyListener(lgrModifyListener);
	}
	...
	protected void actAide() throws Exception {
		...
		setFocusAvantAideSWT(vue.getCompositeSectionParam().getTfCodeArticle());
...
//				case C_MO_CONSULTATION:
					...
					//if(getFocusCourantSWT().equals(vue.getGrille())){
						...
						paramAfficheAideRecherche.setDebutRecherche(vue.getCompositeSectionParam().getTfCodeArticle().getText());

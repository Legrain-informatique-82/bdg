...
import fr.legrain.gestCom.Module_Document.IHMTotauxLignesFacture;
...
public class SWTPaLigneController extends JPABaseControllerSWTStandard
...
//	private SWT_IB_TA_L_FACTURE ibTaTable = new SWT_IB_TA_L_FACTURE(taFacture);
	private TaLFactureDAO dao = null;//new TaLFactureDAO();
	//private WorkbenchPart workbenchPart = null;

	private Object ecranAppelant = null;
	private IHMLFacture ihmLFacture;
	private IHMTotauxLignesDocument ihmTotauxLignesDocument;
	private IHMLFacture ihmOldLFacture;
	private Realm realm;
...
//	private ModelLigneFacture modelLFacture /*=  new ModelLigneFacture(ibTaTable)*/;
	private Class classModel = IHMLFacture.class;
	private ModelGeneralObjet<IHMLFacture,TaLFactureDAO,TaLFacture> modelLFacture /*= new ModelGeneralObjet<IHMLFacture,TaLFactureDAO,TaLFacture>(dao,classModel)*/;
	...
	private LgrDozerMapper<IHMLFacture,TaLFacture> mapperUIToModel = new LgrDozerMapper<IHMLFacture,TaLFacture>();
	private LgrDozerMapper<TaLFacture,IHMLFacture> mapperModelToUI = new LgrDozerMapper<TaLFacture,IHMLFacture>();
	
	private LgrDozerMapper<TaFacture,IHMTotauxLignesDocument> mapperModelToUITotauxLignes = new LgrDozerMapper<TaFacture,IHMTotauxLignesDocument>();
...
//							ibTaTable.initLigneCourantSurRow();
	 ...
	 * @return
	 */
	public List<IHMLFacture> IHMmodel() {
		LinkedList<TaLFacture> ldao = new LinkedList<TaLFacture>();
		LinkedList<IHMLFacture> lswt = new LinkedList<IHMLFacture>();
		
		if(masterEntity!=null && !masterEntity.getLignes().isEmpty()) {

			ldao.addAll(masterEntity.getLignes());

			LgrDozerMapper<TaLFacture,IHMLFacture> mapper = new LgrDozerMapper<TaLFacture,IHMLFacture>();
			for (TaLFacture o : ldao) {
				IHMLFacture t = null;
				t = (IHMLFacture) mapper.map(o, IHMLFacture.class);
				lswt.add(t);
			}
	...
	public void bind(PaSaisieLigneSWT paArticleSWT) {
		try {
			if (modelLFacture==null)modelLFacture = new ModelGeneralObjet<IHMLFacture, TaLFactureDAO, TaLFacture>(dao,classModel);
...
//			setDbcStandard(dbc);
			...
			bindingFormSimple(masterDAO, mapComposantChampsTotauxLignes, dbcTotaux, realm, selectedTotauxLigneFacture, IHMTotauxLignesFacture.class);
...
//			articleContentProposal.setContentProposalProvider(contentProposalProviderArticles());
			...
			articleContentProposal.addContentProposalListener(new IContentProposalListener2(){
				...
				public void proposalPopupOpened(ContentProposalAdapter adapter) {
					...
					if(!dao.dataSetEnModif()&& taLFacture.ligneEstVide()  )
...
//	);
	...
	protected void actChangeTva() throws Exception{
		if(!LibChaine.empty(((IHMLFacture)selectedLigneFacture.getValue()).getCodeArticle())
			...
			InputDialog input = new InputDialog(vue.getShell()
					...
					((IHMLFacture)selectedLigneFacture.getValue()).getCodeTvaLDocument(),new IInputValidator() {
			...
			if(masterDAO.dataSetEnModif()){
				...
				if(input.getValue()!=null ){
					if(MessageDialog.openConfirm(vue.getShell()
						...
						((IHMLFacture)selectedLigneFacture.getValue()).setCodeTvaLDocument(retour);
...
//			}
			if(continuer){
				...
				for (TaLFacture p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument().equals(((IHMLFacture) selectedLigneFacture.getValue()).getNumLigneLDocument())) {
						taLFacture = p;
						taLFacture.setLegrain(true);
						ihmLFacture= IHMLFacture.copy((IHMLFacture) selectedLigneFacture.getValue());;
					}				
				}
				initEtatBouton(false);
...
//				paramAfficheAideRecherche.setDb(getThis().getIbTaTable().getFIBBase());
				...
				switch ((SWTPaLigneController.this.dao.getModeObjet().getMode())) {
				case C_MO_CONSULTATION:
				case C_MO_EDITION:
				case C_MO_INSERTION:
					if (getFocusCourantSWT()==vue.getTfCODE_ARTICLE()){
...
//						paramAfficheAideRecherche.setJPQLQuery("select a from TaArticle a where a.actif = 1 order by codeArticle");
					...
					if (getFocusCourantSWT()==vue.getTfU1_L_FACTURE()||
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						//paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTUnite,TaUniteDAO,TaUnite>(new TaUniteDAO(getEm()).findByArticle(taLFacture.getTaArticle().getCodeArticle()),SWTUnite.class,dao.getEntityManager()));
					...
					if (getFocusCourantSWT()==vue.getTfPRIX_U_L_FACTURE()){
						...
						paramAfficheAideRecherche.setControllerAppelant(getThis());
						paramAfficheAideRecherche.setModel(new ModelGeneralObjet<SWTPrix,TaPrixDAO,TaPrix>(taLFacture.getTaArticle().getTaPrixes(),SWTPrix.class,getEm()));
...
//				dao.begin(transaction);
				if ((dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0)) {
					LgrDozerMapper<IHMLFacture,TaLFacture> mapper = new LgrDozerMapper<IHMLFacture,TaLFacture>();
					mapper.map((IHMLFacture) selectedLigneFacture.getValue(),taLFacture);
...
//			}
	...
	public boolean isUtilise(){
		return ((IHMLFacture)selectedLigneFacture.getValue()).getIdLDocument()!=null && 
		!dao.recordModifiable(dao.getNomTable(),
				((IHMLFacture)selectedLigneFacture.getValue()).getIdLDocument());		
	}

	public IHMLFacture getIhmOldLFacture() {
		return ihmOldLFacture;
	}

	public void setIhmOldLFacture(IHMLFacture swtOldArticle) {
		this.ihmOldLFacture = swtOldArticle;
	}

	public void setIhmOldLFacture() {
		if (selectedLigneFacture!=null && selectedLigneFacture.getValue() != null)
			this.ihmOldLFacture = IHMLFacture.copy((IHMLFacture) selectedLigneFacture.getValue());
		else {
			if(tableViewer!=null && tableViewer.selectionGrille(0)){
				this.ihmOldLFacture = IHMLFacture.copy((IHMLFacture) selectedLigneFacture.getValue());
				tableViewer.setSelection(new StructuredSelection(
						(IHMLFacture) selectedLigneFacture.getValue()), true);
			}}
	}
...
//				mapperUIToModel.map((IHMLFacture)selectedLigneFacture.getValue(), taLFacture);
				for (TaLFacture p : masterEntity.getLignes()) {
					if(p.getNumLigneLDocument()==((IHMLFacture) selectedLigneFacture.getValue()).getNumLigneLDocument()) {
						taLFacture = p;
					}				
				}
			}
	...
	public IHMLFacture getSelectedLigneFacture() {
		IHMLFacture resultat = null;
		if(selectedLigneFacture!=null && selectedLigneFacture.getValue()!=null)
			resultat = (IHMLFacture)selectedLigneFacture.getValue();
		return resultat;
	};	
	...
	protected void insertionLigne(Boolean ajout, boolean initFocus) throws Exception{
		...
		try {
			...
			if(masterDAO.dataSetEnModif()){
			setIhmOldLFacture();
			ihmLFacture = new IHMLFacture();

			dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_INSERTION);
			taLFacture = new TaLFacture(true); 
			taLFacture.setTaTLigne(new TaTLigneDAO(getEm()).findByCode(Const.C_CODE_T_LIGNE_C));
			
			List<IHMLFacture> modelListeLigneFacture = IHMmodel();
...
//								e.doit=false;
			...
			public TaLFactureDAO getDao() {
				return dao;
			}

			public void setDao(TaLFactureDAO dao) {
				this.dao = dao;
			}
			...
			public boolean changementPageValide(){
				if (dao.getModeObjet().getMode().compareTo(ModeObjet.EnumModeObjet.C_MO_EDITION) == 0
					...
					if(taLFacture!=null && selectedLigneFacture!=null && ((IHMLFacture) selectedLigneFacture.getValue())!=null) {
						mapperUIToModel.map((IHMLFacture) selectedLigneFacture.getValue(),taLFacture);
					}
				}
//				dao.getModeObjet().setMode(ModeObjet.EnumModeObjet.C_MO_CONSULTATION);
//				initEtatBouton();
			...
			protected void actSelectionArticle() throws Exception {
				try{
					if((IHMLFacture) selectedLigneFacture.getValue()!=null && ((IHMLFacture) selectedLigneFacture.getValue()).getIdArticle()!=null &&
							((IHMLFacture) selectedLigneFacture.getValue()).getIdArticle()!=0){
					...
					paramAfficheDetail.setIdArticle(String.valueOf(((IHMLFacture) selectedLigneFacture.getValue()).getIdArticle()));

					((AbstractLgrMultiPageEditor)e).findMasterController().configPanel(paramAfficheDetail);
					}
				}catch (Exception e) {
					logger.error("",e);
				}	

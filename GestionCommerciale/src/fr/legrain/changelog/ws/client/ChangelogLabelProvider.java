 ...
 *
 */
public class ChangelogLabelProvider extends LgrViewerSupport {
	...
	static class MyLabelProvider extends ObservableMapLabelProvider implements ITableLabelProvider, ITableColorProvider {
        ...
        public String getColumnText(Object element, int columnIndex) {
        	...
        	if(result instanceof Date) {
    			...
    			return LibDate.dateToString(new Date(new Long((Integer)result)*1000l));
    		} else if (columnIndex==2) { //content
    			String s = ((String)result).replace("<br/>", "\n");
    			s = s.replace("<p>", "");
    			s = s.replace("</p>", "");
    			s = s.replace("<ul>", "");
    			s = s.replace("</ul>", "");
